...
状态：进行中。Reader 默认优先使用 downloaded pages；无默认网络时已 fail-closed 到 offline placeholder，不再生成 source/runtime 或远程 URL 渲染源；本地导入图片仍可读。已有 offline/corrupt/source-index reader smoke。Pura X 已补真实 source 下载后关闭本机 index server、从书架复开 Reader 的可见 UI 证据；也已补损坏/缺页下载从书架复开 Reader 的可见错误占位证据。剩余不是重新实现，而是补更宽设备矩阵：真实断网、未下载章节、路由返回后的可见 UI 证据。
...
超分辨率（MindSpore/NNRT）与漫画翻译（OCR/上下文/渲染管线）是**为 Koma 预做的技术
积累**，在 NextE 已跑通主路径（NNRT 能力探测、模型注册、整页渲染后端、翻译 auto
policy 与本地模型服务）；Koma 侧按自身源/离线数据模型适配，不作为 NextE 功能平移。
这两项放在 Phase 4 之后的独立积累线（见下方路线 Phase 5）。
...
- Phase 5 技术积累落地：图片超分（NNRT 能力探测 → 模型服务 → Reader 渲染接入）与
  漫画翻译（本地模型 → 上下文管线 → 渲染后端），复用 NextE 已验证架构，按 Koma
  source/离线数据模型适配；不占用 Phase 1-4 主线资源。
