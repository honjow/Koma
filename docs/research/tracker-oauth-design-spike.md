...
本设计定义公共追踪服务接入的产品与架构边界。2026-05-26 的 D30 lane 已补上本地账号/授权边界与 OAuth 准备模型，但仍不实现真实网络登录、token 交换、同步写入或已验证安全存储后端。Koma 当前仍应展示诚实的未连接/计划中/安全存储不可用状态，直到安全存储、用户同意、provider 审核和端到端验证完成。
...
当前 App 只允许新增非敏感账号元数据和受控凭据边界；普通持久化和备份仍不得新增原始 token/password/client secret/auth code 字段。
