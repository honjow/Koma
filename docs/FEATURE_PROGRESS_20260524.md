...
| Source offline reader hash freeze fix | `current` | Pura X real MangaDex `Salt Friend` download/Reader smoke exposed main-thread full-chapter content hashing in download completion and offline page lookup; reader/download completion now use lightweight structural validation while strict hash validation remains the default for explicit integrity checks. |
...
- Source UI main path has Pura X evidence with real MangaDex data: Browse -> MangaDex -> `Salt Friend` detail -> Reader. Detail rendered MangaDex metadata, 99 chapters, description/tags, and Reader opened `Salt Friend · Cooling down. (3)` at `1 / 13`. Artifacts: `.hvigor/outputs/source-ui-route-smoke/source-after-install.png`, `.hvigor/outputs/source-ui-route-smoke/detail-screen-fixed.png`, `.hvigor/outputs/source-ui-route-smoke/reader-attempt-screen.png`.
- Source reader smoke is now repeatable through `scripts/run_source_reader_smoke.sh`: it builds/installs Koma, serves a user-configured URL source index, installs `org.mangadex.koma`, searches `Salt Friend`, writes the source manga into the visible library, captures the library item, opens the continue-reading card, and captures Reader. Latest Pura X simulator evidence opened `Salt Friend · Cooling down. (3)` at `2 / 13`. Artifacts: `.hvigor/outputs/source-index-visible-reader-ui/library-screen.png`, `.hvigor/outputs/source-index-visible-reader-ui/reader-screen.png`.
- Source-backed offline download and Reader have Pura X evidence with the same real MangaDex `Salt Friend` chapter. A prior run reproduced `THREAD_BLOCK_6S` in `createOfflineDownloadFileContentHash` during download completion and then offline reader page lookup; after the lightweight validation fix, Koma opened `Salt Friend · Cooling down. (3)` at `1 / 13`, advanced to `2 / 13`, and no appfreeze newer than the pre-fix `2026-07-05 06:09` faultlog was present. Artifacts: `.hvigor/outputs/source-download-smoke/fixed-reader2.json`, `.hvigor/outputs/source-download-smoke/fixed-reader2.png`, `.hvigor/outputs/source-download-smoke/fixed-reader-page2.json`, `.hvigor/outputs/source-download-smoke/fixed-reader-page2.png`, plus pre-fix appfreeze logs in the same directory.
