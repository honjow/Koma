...
assertExport(localImportCoordinatorSource, 'createFolderDocumentSelectOptions')
assertExport(localImportCoordinatorSource, 'pickArchiveUris')
assertExport(localImportCoordinatorSource, 'pickImageUris')
assertExport(localImportCoordinatorSource, 'pickFolderUris')
assertExport(localImportCoordinatorSource, 'scanPickedFolderUri')
...
const archiveDocumentOptionsSource = localImportCoordinatorSource.match(/export function createArchiveDocumentSelectOptions[\s\S]*?return options\n\}/)?.[0] ?? ''
assert.doesNotMatch(archiveDocumentOptionsSource, /selectMode|DocumentSelectMode\.FOLDER/, 'archive picker must not request folder selection')
assert.match(localImportCoordinatorSource, /export function createFolderDocumentSelectOptions[\s\S]*options\.selectMode = picker\.DocumentSelectMode\.FOLDER[\s\S]*options\.maxSelectNumber = maxSelectNumber/, 'folder picker must explicitly request folder selection')
assert.match(localImportCoordinatorSource, /fs\.listFile\(rootUri, listFileOptions\)[\s\S]*scanLocalLibraryFolderEntries\(entries, createPickedFolderRootName\(rootUri\)\)/, 'picked folder import must enumerate the returned folder URI and reuse the local library folder scanner')
...
assert.match(importPageSource, /export interface LocalFolderImportPersistResult[\s\S]*importedCount: number[\s\S]*checkedCount: number[\s\S]*missingCount: number/, 'ImportPage must expose structured local folder import persistence results')
assert.match(importPageSource, /onLocalFolderImportSucceeded:\s*\(scanResult:\s*LocalLibraryFolderScanResult,\s*rootUri:\s*string\)\s*=>\s*LocalFolderImportPersistResult/, 'ImportPage must expose a successful local folder scan callback')
assert.match(importPageSource, /const results = await this\.localImportCoordinator\.pickAndImportArchives\(context, 1\)[\s\S]*tf\('import_feedback_partial', succeededCount, failedCount\)/, 'ImportPage must summarize archive successes and failures')
...
assert.match(importPageSource, /const results = await this\.localImportCoordinator\.pickAndScanFolders\(context, 1\)[\s\S]*const persistResult = this\.onLocalFolderImportSucceeded\(result\.scanResult, result\.rootUri\)[\s\S]*checkedCount \+= persistResult\.checkedCount[\s\S]*missingCount \+= persistResult\.missingCount/, 'ImportPage must scan selected folders and delegate structured persistence summaries to the app shell')
assert.match(importPageSource, /import_feedback_folder_rescan_done[\s\S]*import_feedback_folder_rescan_missing[\s\S]*import_feedback_folder_rescan_unchanged/, 'ImportPage must distinguish local folder rescan changed, missing, and unchanged outcomes')
assert.match(importPageSource, /function importDebugEventSummary\(event: LocalArchiveImportDebugEvent\)[\s\S]*errorState[\s\S]*redacted[\s\S]*uris=\$\{event\.uriCount \?\? 0\} pages=\$\{event\.pageCount \?\? 0\} error=\$\{errorState\}/, 'ImportPage debug logs must keep aggregate import signals while redacting error details')
assert.doesNotMatch(importPageSource, /console\.(?:info|warn|error)\([^\n]*(?:path=|sourceUri|sandboxZipPath|cacheRootDir|extractionDir|message=|error=['"]?\s*\+|`\$\{error\}`|\.message)/, 'ImportPage logs must not expose picker paths, cache paths, raw errors, or raw Error.message')
assert.match(importPageSource, /archive_import_failed code=archive_import_failed[\s\S]*image_import_failed code=image_import_failed[\s\S]*folder_import_persist_failed code=library_persist_failed[\s\S]*folder_import_failed code=folder_import_failed/, 'ImportPage failure logs must use fixed redacted codes')
...
assert.match(indexPageSource, /private refreshLibrarySnapshot\(\):\s*void \{[\s\S]*this\.libraryComics = this\.libraryStore\.listComics\(\)[\s\S]*this\.libraryRevision \+= 1[\s\S]*\}/, 'Index must publish a fresh shelf snapshot while bumping the explicit refresh signal')
assert.match(indexPageSource, /private handleArchiveImportSucceeded\(comic:\s*Comic\):\s*void \{[\s\S]*(this\.libraryStore\.upsertComic\(comic\)|upsertComicAndPersistLibraryStore\(this\.libraryStore, persistenceService, comic\))[\s\S]*catch \(error\) \{[\s\S]*step=persist_imported_comic_failed code=library_persist_failed[\s\S]*throw error as Error[\s\S]*this\.refreshLibrarySnapshot\(\)[\s\S]*this\.selectedTab = 0[\s\S]*\}/, 'Index must make persistence failures visible with redacted codes and only refresh shelf success state after import persistence succeeds')
assert.doesNotMatch(indexPageSource, /persist_imported_(comic|folder)_failed[^\n]*(message=|\.message|failed: ' \+)/, 'Index import persistence logs must not expose raw error messages')
assert.match(indexPageSource, /private handleLocalFolderImportSucceeded\(scanResult:\s*LocalLibraryFolderScanResult,\s*rootUri:\s*string\):\s*LocalFolderImportPersistResult \{[\s\S]*upsertLocalLibraryFolderScanAndPersistLibraryStore\([\s\S]*scanResult,[\s\S]*rootUri[\s\S]*upsertLocalLibraryFolderRescanAndPersistLibraryStore[\s\S]*this\.refreshLibrarySnapshot\(\)[\s\S]*this\.selectedTab = 0[\s\S]*return \{[\s\S]*importedCount,[\s\S]*checkedCount,[\s\S]*missingCount,[\s\S]*\}/, 'Index must persist local folder scans and rescans through the local library folder helpers')
assert.match(indexPageSource, /aboutToAppear\(\):\s*void \{[\s\S]*persistenceService\.restore\(\)[\s\S]*this\.refreshLibrarySnapshot\(\)/, 'Index must hydrate persisted library data during startup before shelf rendering where feasible')
...
assert.match(indexPageSource, /onLocalFolderImportSucceeded:\s*\(scanResult:\s*LocalLibraryFolderScanResult,\s*rootUri:\s*string\):\s*LocalFolderImportPersistResult => \{[\s\S]*this\.handleLocalFolderImportSucceeded\(scanResult, rootUri\)/, 'Index must pass local folder scans into the app shell persistence path')
assert.match(indexPageSource, /LibraryPage\(\{[\s\S]*libraryComics:\s*this\.libraryComics[\s\S]*libraryRevision:\s*this\.libraryRevision/, 'Index must pass explicit shelf data and refresh signal into LibraryPage')
