...
assert.match(importPageSource, /const results = await this\.localImportCoordinator\.pickAndImportArchives\(context, 1\)[\s\S]*已导入 \$\{succeededCount\} 本，失败 \$\{failedCount\} 个/, 'ImportPage must summarize archive successes and failures')
