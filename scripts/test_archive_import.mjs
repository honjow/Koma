...
assert.match(importPageSource, /onLocalFolderImportSucceeded:\s*\(scanResult:\s*LocalLibraryFolderScanResult,\s*rootUri:\s*string\)\s*=>\s*number/, 'ImportPage must expose a successful local folder scan callback')
...
assert.match(importPageSource, /const results = await this\.localImportCoordinator\.pickAndScanFolders\(context, 1\)[\s\S]*this\.onLocalFolderImportSucceeded\(result\.scanResult, result\.rootUri\)/, 'ImportPage must scan selected folders and delegate persistence to the app shell')
...
assert.match(indexPageSource, /private handleLocalFolderImportSucceeded\(scanResult:\s*LocalLibraryFolderScanResult,\s*rootUri:\s*string\):\s*number \{[\s\S]*upsertLocalLibraryFolderScanAndPersistLibraryStore\([\s\S]*scanResult,[\s\S]*rootUri[\s\S]*this\.refreshLibrarySnapshot\(\)[\s\S]*this\.selectedTab = 0[\s\S]*return importedCount[\s\S]*\}/, 'Index must persist local folder scans through the local library folder helper')
...
assert.match(indexPageSource, /onLocalFolderImportSucceeded:\s*\(scanResult:\s*LocalLibraryFolderScanResult,\s*rootUri:\s*string\):\s*number => \{[\s\S]*this\.handleLocalFolderImportSucceeded\(scanResult, rootUri\)/, 'Index must pass local folder scans into the app shell persistence path')
