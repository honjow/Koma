...
assert.match(managerPageSource, /descriptor\.kind === 'range'[\s\S]*this\.SettingRangeRow\(descriptor\)/, 'SourcePackageManagerPage must render safe range source settings as numeric controls')
assert.match(managerPageSource, /adjustRangeSetting\(descriptor: SourceSettingDescriptor, direction: number\)[\s\S]*this\.rangeStep\(descriptor\) \* direction[\s\S]*this\.normalizeRangeSetting\(descriptor[\s\S]*this\.updateSettingValue\(descriptor\.id, next\)/, 'SourcePackageManagerPage range settings must step numeric values through normalized min and max bounds')
