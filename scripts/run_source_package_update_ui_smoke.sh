...
artifact_dir="${KOMA_SOURCE_UPDATE_UI_ARTIFACT_DIR:-.hvigor/outputs/source-package-update-ui-smoke}"

mkdir -p "$artifact_dir"

if [ ! -x "$hdc" ]; then
  echo "source package update UI smoke failed: hdc not found or not executable: $hdc" >&2
  exit 1
fi
...
wait_layout_contains_any() {
  local name="$1"
  shift
  local poll_count="${KOMA_SOURCE_UPDATE_UI_POLL_COUNT:-12}"
  local poll_delay="${KOMA_SOURCE_UPDATE_UI_POLL_DELAY_SECONDS:-2}"
...
PY
  ...
  echo "source package update UI smoke failed: timed out waiting for $*" >&2
  return 1
}
...
sleep "${KOMA_SOURCE_UPDATE_UI_START_WAIT_SECONDS:-5}"

capture_layout "source-update-manager"
wait_layout_contains_any "source-update-manager-ready" "$source_display"
click_from_layout "$artifact_dir/source-update-manager-ready-layout.json" "$artifact_dir/click-check-updates.txt" "Check updates" "检查更新"
wait_layout_contains_any "source-update-checked" "Updates: latest" "更新：已是最新"

echo "source package update UI smoke passed: $artifact_dir"
