...
assert.match(readerPageSource, /readerPlaceholderShowsReturnAction\(source: ReaderPageRenderSource\): boolean[\s\S]*ReaderPageUnavailableReason\.OFFLINE_MISSING/, 'ReaderPage must show a recovery action only for offline missing chapter pages')
...
assert.match(readerPageSource, /PageErrorPlaceholderContent\(index: number, compact: boolean = false, detail: string = s\('reader_page_unavailable'\), showReturnAction: boolean = false\)[\s\S]*Text\(detail\)/, 'reader error placeholder must render explicit failure detail copy')
...
assert.match(readerPageSource, /remoteImageFailureDetail\(source: ReaderPageRenderSource\): string \{[\s\S]*source\.sourceRuntimeId !== undefined[\s\S]*reader_source_page_unavailable[\s\S]*reader_remote_page_unavailable[\s\S]*RemoteImagePage\(source: ReaderPageRenderSource[\s\S]*this\.hasImageLoadFailed\(source\.imageUri, source, splitSide\)[\s\S]*this\.PageErrorPlaceholder\(index, compact, this\.remoteImageFailureDetail\(source\)\)/, 'source-runtime reader failures must show source-specific recovery copy while ordinary remote failures keep offline/network copy')
assert.match(readerPageSource, /LocalImagePage\(imageUri: string[\s\S]*this\.hasImageLoadFailed\(imageUri, undefined, splitSide\)[\s\S]*this\.PageErrorPlaceholder\(index, compact, s\('reader_local_page_unavailable'\)\)/, 'local reader failures must not masquerade as remote network success')
...
assert.match(indexSource, /const nextSessionConfig = comic === undefined \? MOCK_LIBRARY_READER_SESSION : this\.createReaderSessionConfigForOpen\(comic, targetChapterId\)[\s\S]*this\.readerSessionConfig = nextSessionConfig/, 'all reader entry points must share the offline-manifest-aware reader session creation path')
...
assert.match(readerPageSource, /PageErrorPlaceholderContent\(index: number[\s\S]*Text\(s\('reader_page_load_failed'\)\)[\s\S]*Text\(detail\)[\s\S]*maxLines\(compact \? 2 : 3\)[\s\S]*Text\(s\('reader_page_number'\)\.replace\('%s', `\$\{index \+ 1\}`\)\)/, 'reader error placeholders must show failure copy first and keep the page number as small metadata')
...
assert.match(readerPageSource, /private refreshReaderSurfaceSize\(areaWidthVp: number = 0, areaHeightVp: number = 0\): void \{[\s\S]*const nextWidthVp = areaWidthVp > 0 \? areaWidthVp : this\.readDefaultDisplayWidthVp\(\)[\s\S]*const nextHeightVp = areaHeightVp > 0 \?[\s\S]*this\.readDefaultDisplayHeightVp\(\) - this\.readerContentTopInset\(\) - this\.readerContentBottomInset\(\)/, 'reader measured content surface must win over full display fallback for zoom bounds and dual-page decisions')
assert.match(readerPageSource, /private viewportHeight\(\): number \{[\s\S]*if \(this\.readerSurfaceHeightVp > 0\) \{[\s\S]*return this\.readerSurfaceHeightVp[\s\S]*this\.readDefaultDisplayHeightVp\(\) - this\.readerContentTopInset\(\) - this\.readerContentBottomInset\(\)/, 'measured reader content height must not subtract safe areas a second time')
assert.match(readerPageSource, /private ReaderSafeContentFrame\(\)[\s\S]*this\.ReaderSafeAreaSpacer\(this\.readerContentTopInset\(\)\)[\s\S]*this\.ReaderInteractiveContentSurface\(\)[\s\S]*this\.ReaderSafeAreaSpacer\(this\.readerContentBottomInset\(\)\)/, 'reader safe-area spacers must sit outside the measured interactive reading surface')
assert.match(readerPageSource, /private ReaderInteractiveContentSurface\(\)[\s\S]*this\.ReaderContentViewport\(\)[\s\S]*\.scale\(\{ x: this\.zoomScale, y: this\.zoomScale \}\)[\s\S]*\.translate\(\{ x: this\.zoomOffsetX, y: this\.zoomOffsetY \}\)[\s\S]*this\.ReaderInputLayer\(\)[\s\S]*\.onAreaChange\(\(oldValue: Area, newValue: Area\) => \{[\s\S]*this\.onReaderAreaChanged\(oldValue, newValue\)/, 'reader measured safe content surface must own the scaled viewport and top input layer')
...
assert.match(readerPreferencesStoreSource, /DEFAULT_READER_PREFERENCES:[\s\S]*backgroundMode:\s*'black'[\s\S]*showProgressControls:\s*true[\s\S]*keepScreenAwake:\s*true/, 'reader settings MVP defaults must preserve the current dark reader, visible progress UI, and reader keep-awake UX')
