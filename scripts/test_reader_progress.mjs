...
assert.match(readerPageSourceAdapterSource, /import \{ sourceRuntimeEntryEnabled \} from '\.\.\/sourceRuntime\/SourceRuntimeRegistry'/, 'reader source adapter must use the shared source enabled-state guard')
assert.match(readerPageSourceAdapterSource, /function createReaderSourceUnavailableSource\(config: ReaderSessionConfig, pageIndex: number, sourceRuntimeId: string, pageId: string\): ReaderPageRenderSource[\s\S]*ReaderPageRenderKind\.URI_PLACEHOLDER[\s\S]*ReaderPageUnavailableReason\.SOURCE_UNAVAILABLE/, 'reader source adapter must expose source-runtime unavailable pages as explicit source failure placeholders')
assert.match(readerPageSourceAdapterSource, /function createReaderSourceRuntimeRenderSource\(config: ReaderSessionConfig, pageIndex: number\)[\s\S]*const sourceRuntimeId = config\.sourceRuntimeId\.trim\(\)[\s\S]*if \(isReaderLocalImageSourceUri\(uri\)\) \{[\s\S]*return undefined[\s\S]*const lookup = appSourceRuntimeRegistry\.lookup\(sourceRuntimeId\)[\s\S]*!sourceRuntimeEntryEnabled\(lookup\.entry\)[\s\S]*!isReaderRemoteImageSourceUri\(uri\)[\s\S]*return createReaderSourceUnavailableSource\(config, pageIndex, sourceRuntimeId, pageId\)[\s\S]*const fallbackImageUri = isReaderRemoteImageSourceUri\(uri\) \? uri : `source-runtime:\/\/\$\{sourceRuntimeId\}\/\$\{encodeURIComponent\(pageId\)\}`/, 'reader source runtime resolution must fail early only when source runtime is unavailable and the page has no direct local or remote fallback')
...
assert.match(readerPageSource, /@Event onOpenMangaDetail: \(\) => void[\s\S]*KomaActionButton\(\{[\s\S]*reader_action_return_to_detail[\s\S]*this\.onOpenMangaDetail\(\)/, 'offline missing reader placeholder must offer a real manga-detail recovery action instead of only closing the reader')
...
assert.match(indexSource, /createOfflineDownloadReaderSessionConfig\([\s\S]*new OfflineDownloadStore\(context\.filesDir\)[\s\S]*validateDownloadedChapter\(comic\.id, chapterId, \{ validateContentHash: false \}\)[\s\S]*const hasReaderReadyPages = validation\.availablePageCount > 0[\s\S]*OfflineDownloadedChapterStatus\.DOWNLOADED[\s\S]*OfflineDownloadedChapterStatus\.PARTIAL[\s\S]*const hasCorruptManifestPageMap = validation\.status === OfflineDownloadedChapterStatus\.CORRUPT[\s\S]*validation\.pageCount > 0[\s\S]*!hasReaderReadyPages && !hasCorruptManifestPageMap[\s\S]*return undefined/, 'offline manifest reader sessions must use reader-ready pages and preserve corrupt manifest page maps as honest offline placeholders')
...
assert.match(readerPageSource, /private ReaderInputLayer\(\)[\s\S]*\.zIndex\(12\)[\s\S]*\.hitTestBehavior\(HitTestMode\.Block\)[\s\S]*GestureGroup\(GestureMode\.Parallel[\s\S]*GestureGroup\(GestureMode\.Exclusive[\s\S]*TapGesture\(\{ count: 2, fingers: 1, distanceThreshold: READER_TAP_MOVE_TOLERANCE_VP \}\)[\s\S]*onReaderDoubleTap[\s\S]*TapGesture\(\{ count: 1, fingers: 1, distanceThreshold: READER_TAP_MOVE_TOLERANCE_VP \}\)[\s\S]*onReaderTap[\s\S]*PinchGesture\(\{ fingers: 2 \}\)[\s\S]*onReaderPinchStart[\s\S]*onReaderPinchUpdate[\s\S]*onReaderPinchEnd[\s\S]*PanGesture\(\{ fingers: 1, direction: PanDirection\.All, distance: 2 \}\)[\s\S]*onReaderPanStart[\s\S]*onReaderPanUpdate[\s\S]*onReaderPanEnd/, 'reader gestures and tap input must share the full input surface below chrome instead of letting chrome or tap overlays block zoom gestures')
assert.match(readerPageSource, /ReaderChrome\(\{[\s\S]*\.zIndex\(20\)[\s\S]*\.hitTestBehavior\(this\.activeChromeVisible \? HitTestMode\.Transparent : HitTestMode\.None\)/, 'reader chrome wrapper must not swallow reader taps when hidden and must pass middle-surface taps through while visible')
assert.match(readerPageSource, /const READER_SWIPE_PAGE_THRESHOLD_VP: number = 72[\s\S]*const READER_SWIPE_VERTICAL_REJECT_RATIO: number = 1\.35[\s\S]*private handleReaderSwipeCandidate\(point: ReaderTouchPoint\): boolean \{[\s\S]*!this\.swipeNavigationEnabled[\s\S]*this\.zoomScale > 1\.01[\s\S]*this\.currentReaderMode\(\) === ReaderMode\.CONTINUOUS_SCROLL[\s\S]*READER_SWIPE_PAGE_THRESHOLD_VP[\s\S]*READER_SWIPE_VERTICAL_REJECT_RATIO[\s\S]*step=swipe_navigation[\s\S]*this\.nextPage\(\)[\s\S]*this\.previousPage\(\)/, 'reader touch layer must preserve unzoomed horizontal swipe page navigation without hijacking zoomed pan or webtoon scroll')
...
assert.match(readerPageSource, /private shouldFitReaderHeight\(\): boolean \{[\s\S]*this\.imageFitMode === 'fit_height' && this\.currentReaderMode\(\) !== ReaderMode\.CONTINUOUS_SCROLL[\s\S]*fitReaderZoomContentSizeToViewport\(aspectRatio: number\)[\s\S]*if \(this\.shouldFitReaderHeight\(\)\) \{[\s\S]*width: Math\.max\(1, viewportHeight \* aspectRatio\)[\s\S]*height: Math\.max\(1, viewportHeight\)/, 'reader fit-height mode must size non-continuous pages from viewport height while leaving continuous scroll width-based')
...
assert.doesNotMatch(readerChromeSource, /private MiddleDismissSurface|HitTestMode\.Block[\s\S]*hideChrome|backgroundColor\('#01000000'\)/, 'ReaderChrome must not block the middle reading surface; reader taps and zoom gestures belong to ReaderInputLayer')
assert.match(readerChromeSource, /build\(\) \{[\s\S]*Stack\(\{ alignContent: Alignment\.TopStart \}\) \{[\s\S]*this\.TopBar\(\)[\s\S]*Column\(\{ space: 0 \}\)[\s\S]*this\.QuickSettingsPanel\(\)[\s\S]*this\.BottomBar\(\)[\s\S]*\.align\(Alignment\.Bottom\)/, 'ReaderChrome controls must keep the middle reading surface non-intercepting while pinning controls to top and bottom')
