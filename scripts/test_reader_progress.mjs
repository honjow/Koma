...
assert.match(indexSource, /this\.readerSessionConfig = comic === undefined \? MOCK_LIBRARY_READER_SESSION : this\.createReaderSessionConfigForOpen\(comic, chapterId\)/, 'all reader entry points must share the offline-manifest-aware reader session creation path')
...
assert.match(readerSessionStoreSource, /chapterIds: comic\.chapters\.map\(\(item: Chapter\): string => item\.id\)/, 'reader session config creation must preserve series chapter ordering')
