...
assert.match(readerPageSourceAdapterSource, /export function createReaderPageRenderSource[\s\S]*createReaderOfflineDownloadRenderSource\(config, pageIndex\)[\s\S]*return offlineDownloadSource[\s\S]*createReaderSourceRuntimeRenderSource/, 'reader page render source must prefer offline local files before source/runtime remote fallback')
