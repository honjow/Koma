...
assert.match(offlineDownloadStoreSource, /resolveDownloadedPage[\s\S]*OfflineDownloadStatus\.DOWNLOADED[\s\S]*OfflineDownloadStatus\.PARTIAL[\s\S]*fs\.accessSync\(page\.localPath\)/, 'offline resolver must allow existing local pages from partial downloads')
