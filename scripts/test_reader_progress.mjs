...
assert.match(settingsPageSource, /configureReaderRemoteImageCache\(context\.cacheDir, this\.imageCacheMaxBytes\)/, 'Settings must configure the reader remote image cache with the selected limit before stats or clear')
assert.match(readerPageSource, /ReaderRemoteImageCachePreferencesStore\(context\)\.load\(\)[\s\S]*configureReaderRemoteImageCache\(cacheDir, preferences\.maxBytes\)/, 'ReaderPage must apply persisted cache limit before using the remote image cache')
