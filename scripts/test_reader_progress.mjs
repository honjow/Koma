...
assert.match(readerPageSource, /private ReaderInteractiveContentSurface\(\)[\s\S]*this\.ReaderContentViewport\(\)[\s\S]*\.scale\(\{ x: this\.zoomScale, y: this\.zoomScale \}\)[\s\S]*PinchGesture\(\{ fingers: 2 \}\)[\s\S]*PanGesture\(\{ fingers: 1, direction: PanDirection\.All, distance: 2 \}\)[\s\S]*this\.ReaderInputLayer\(\)[\s\S]*\.onAreaChange\(\(oldValue: Area, newValue: Area\) => \{[\s\S]*this\.onReaderAreaChanged\(oldValue, newValue\)/, 'reader gestures and tap input must share the measured safe content surface instead of the full system-safe root')
