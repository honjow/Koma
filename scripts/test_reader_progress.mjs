...
assert.match(readerPageSource, /RemoteImagePage\(source: ReaderPageRenderSource[\s\S]*this\.hasImageLoadFailed\(source\.imageUri, source, splitSide\)[\s\S]*this\.PageErrorPlaceholder\(index, compact, s\('reader_remote_page_unavailable'\)\)/, 'undownloaded remote reader failures must show honest offline/network unavailable copy without poisoning other source pages that reuse a URI')
