...
assert.match(readerPageSource, /remoteImageFailureDetail\(source: ReaderPageRenderSource\): string \{[\s\S]*source\.sourceRuntimeId !== undefined[\s\S]*reader_source_page_unavailable[\s\S]*reader_remote_page_unavailable[\s\S]*RemoteImagePage\(source: ReaderPageRenderSource[\s\S]*this\.hasImageLoadFailed\(source\.imageUri, source, splitSide\)[\s\S]*this\.PageErrorPlaceholder\(index, compact, this\.remoteImageFailureDetail\(source\), false, splitSide\)/, 'source-runtime reader failures must show source-specific recovery copy while ordinary remote failures keep offline/network copy')
assert.match(readerPageSource, /LocalImagePage\(imageUri: string[\s\S]*this\.hasImageLoadFailed\(imageUri, undefined, splitSide\)[\s\S]*this\.PageErrorPlaceholder\(index, compact, s\('reader_local_page_unavailable'\), false, splitSide\)/, 'local reader failures must not masquerade as remote network success')
