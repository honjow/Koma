...
assert.match(libraryCategoryPage, /MenuItem\(\{ content: '上移' \}\)[\s\S]*enabled\(!this\.isFirstCategory\(category\)\)[\s\S]*persistMove\(category\.id, 'up'\)[\s\S]*MenuItem\(\{ content: '下移' \}\)[\s\S]*enabled\(!this\.isLastCategory\(category\)\)[\s\S]*persistMove\(category\.id, 'down'\)/, 'category management page must expose bounded up/down controls')
assert.match(libraryCategoryPage, /setCategoryDisplayStrategyAndPersistLibraryStore[\s\S]*DisplayStrategySection\(\)[\s\S]*StrategySummaryRow\('all', '全部分类'\)[\s\S]*StrategySummaryRow\('uncategorized', '未分类'\)[\s\S]*StrategySummaryRow\(LIBRARY_CATEGORY_FAVORITE_ID, '收藏'\)/, 'category management page must expose category display strategy controls from settings')
