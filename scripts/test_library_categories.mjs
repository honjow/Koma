...
assert.doesNotMatch(libraryPage, /import \{ Chip, ChipSize \} from '@kit\.ArkUI'|\bChip\(/, 'LibraryPage filter controls must not fall back to system chips')
assert.doesNotMatch(libraryPage, /`\$\{this\.(sourceLabel|readStateLabel|availabilityLabel|metadataStatusLabel|categoryLabel)\(\)\} \u25BE`|`\$\{s\('library_action_categories'\)\} \u25BE`|`\$\{s\('library_category_read_state_title'\)\} \u25BE`/, 'LibraryPage menu buttons must use symbol icons instead of handwritten arrow characters')
assert.match(libraryPage, /import \{ KomaMenuChip \} from '..\/components\/ui\/KomaMenuChip'[\s\S]*SortFilterBar\(\)[\s\S]*KomaMenuChip\(\{ label: s\('library_sort_chip'\)\.replace\('%s', this\.sortLabel\(\)\), active: this\.sortBy !== 'lastRead' \}\)[\s\S]*KomaMenuChip\(\{ label: this\.sourceLabel\(\), active: this\.filterSource !== 'all' \}\)[\s\S]*KomaMenuChip\(\{ label: this\.readStateLabel\(\), active: this\.filterReadState !== 'all' \}\)[\s\S]*KomaMenuChip\(\{ label: this\.availabilityLabel\(\), active: this\.filterAvailability !== 'all' \}\)[\s\S]*KomaMenuChip\(\{ label: this\.metadataStatusLabel\(\), active: this\.filterMetadataStatus !== 'all' \}\)[\s\S]*KomaMenuChip\(\{ label: this\.categoryLabel\(\), active: this\.filterCategory !== 'all' \}\)/, 'LibraryPage filter bar must render compact shared menu chips with real symbol glyphs')
