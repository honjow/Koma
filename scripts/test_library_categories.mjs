...
assert.match(libraryCategoryPage, /setCategoryDisplayStrategyAndPersistLibraryStore[\s\S]*DisplayStrategySection\(\)[\s\S]*StrategyRow\('all', '全部分类'\)[\s\S]*StrategyRow\('uncategorized', '未分类'\)[\s\S]*StrategyRow\(LIBRARY_CATEGORY_FAVORITE_ID, '收藏'\)/, 'category management page must expose category display strategy controls from settings')
...
assert.match(libraryPage, /MenuItem\(\{ content: '加入收藏' \}\)[\s\S]*addSelectedCategory\(LIBRARY_CATEGORY_FAVORITE_ID\)[\s\S]*MenuItem\(\{ content: '移出收藏' \}\)[\s\S]*removeSelectedCategory\(LIBRARY_CATEGORY_FAVORITE_ID\)/, 'batch actions must add and remove Favorite')
assert.match(libraryPage, /MenuItem\(\{ content: '加入稍后' \}\)[\s\S]*addSelectedCategory\(LIBRARY_CATEGORY_READ_LATER_ID\)[\s\S]*MenuItem\(\{ content: '移出稍后' \}\)[\s\S]*removeSelectedCategory\(LIBRARY_CATEGORY_READ_LATER_ID\)/, 'batch actions must add and remove Read Later')
assert.match(libraryPage, /MenuItem\(\{ content: '清除分类' \}\)[\s\S]*assignSelectedCategory\(undefined\)/, 'batch actions must keep clear category assignment')
assert.match(libraryPage, /Button\('删除'\)[\s\S]*showBatchRemoveConfirmation\(\)/, 'existing multi-select remove action must remain available')
