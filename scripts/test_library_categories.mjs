...
assert.doesNotMatch(libraryPage, /import \{ Chip, ChipSize \} from '@kit\.ArkUI'|Chip\(/, 'LibraryPage filter controls must use shared Koma buttons instead of system chips')
assert.doesNotMatch(libraryPage, /`\$\{this\.(sourceLabel|readStateLabel|availabilityLabel|metadataStatusLabel|categoryLabel)\(\)\} ▾`|`\$\{s\('library_action_categories'\)\} ▾`|`\$\{s\('library_category_read_state_title'\)\} ▾`/, 'LibraryPage menu buttons must use symbol chevrons instead of handwritten arrow characters')
assert.match(libraryPage, /SortFilterBar\(\)[\s\S]*KomaActionButton\(\{[\s\S]*label: s\('library_sort_chip'\)\.replace\('%s', this\.sortLabel\(\)\)[\s\S]*icon: \$r\('sys\.symbol\.chevron_down'\)[\s\S]*label: this\.sourceLabel\(\)[\s\S]*label: this\.readStateLabel\(\)[\s\S]*label: this\.availabilityLabel\(\)[\s\S]*label: this\.metadataStatusLabel\(\)[\s\S]*label: this\.categoryLabel\(\)/, 'LibraryPage filter bar must render shared menu buttons with symbol chevrons')
