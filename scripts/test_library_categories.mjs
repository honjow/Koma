...
assert.match(libraryPage, /downloadedComicAvailability\(\): Map<ComicId, OfflineDownloadStatus>[\s\S]*entry\.status === OfflineDownloadStatus\.PARTIAL[\s\S]*comicAvailability\.set\(entry\.comicId, OfflineDownloadStatus\.PARTIAL\)[\s\S]*entry\.status === OfflineDownloadStatus\.DOWNLOADED[\s\S]*comicAvailability\.get\(entry\.comicId\) !== OfflineDownloadStatus\.PARTIAL/, 'LibraryPage download availability aggregation must keep partially downloaded comics out of the fully downloaded filter')
...
assert.match(libraryPage, /@Local private listDensity: LibraryListDensity = 'standard'[\s\S]*this\.listDensity = preferencesValue\.listDensity[\s\S]*listDensity: this\.listDensity[\s\S]*libraryGridColumnsTemplate\(\)[\s\S]*this\.listDensity === 'compact'[\s\S]*'1fr 1fr 1fr 1fr'[\s\S]*this\.listDensity === 'comfortable'[\s\S]*'1fr 1fr'[\s\S]*columnsTemplate\(this\.libraryGridColumnsTemplate\(\)\)[\s\S]*Column\(\{ space: this\.libraryListSpace\(\) \}/, 'LibraryPage must restore, persist, and apply list density to grid and list rendering')
