...
if [ -f "$KOMA_REPO/scripts/dev.env" ]; then
  # shellcheck disable=SC1091
  source "$KOMA_REPO/scripts/dev.env"
else
  cat > "$LANE_ART/result.json" <<JSON
{"verdict":"BLOCKED","blocked_reason":"missing_dev_env","artifact_dir":"$LANE_ART","hint":"$KOMA_REPO/scripts/dev.env not found"}
JSON
  echo "lane-preflight: $KOMA_REPO/scripts/dev.env not found → BLOCKED" >&2
  exit 1
fi

# 3) 签名 token 预检（绕过 headless browser login 死路）
"$KOMA_REPO/scripts/check-signing-auth.sh" >>"$LANE_ART/preflight.log" 2>&1
auth_code=$?
case "$auth_code" in
  0) ;;
  2)
