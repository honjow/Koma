...
assert.match(kavitaBrowseSource, /KAVITA_SERIES_PAGE_SIZE: number = 50[\s\S]*openLibrary\(item: KavitaLibraryDto\)[\s\S]*listSeries\(item\.id, 1, KAVITA_SERIES_PAGE_SIZE\)[\s\S]*this\.seriesHasMore = rows\.length >= KAVITA_SERIES_PAGE_SIZE[\s\S]*SeriesRow\(item: KavitaSeriesDto\)/, 'Kavita browse page must drill from libraries into a paged series list without faking reader support')
assert.match(kavitaBrowseSource, /appendUniqueSeries\(current: KavitaSeriesDto\[\], next: KavitaSeriesDto\[\]\)[\s\S]*series\.id === item\.id[\s\S]*loadMoreSeries\(\)[\s\S]*const nextPage = this\.seriesPage \+ 1[\s\S]*listSeries\(library\.id, nextPage, KAVITA_SERIES_PAGE_SIZE\)[\s\S]*this\.series = this\.appendUniqueSeries\(this\.series, rows\)[\s\S]*this\.seriesHasMore = rows\.length >= KAVITA_SERIES_PAGE_SIZE[\s\S]*common_loading_more[\s\S]*common_load_more[\s\S]*isEnabled: !this\.seriesLoadingMore/, 'Kavita browse page must support loading more series for large private libraries')
...
assert.match(kavitaBrowseSource, /isSelectedSeriesInLibrary\(\)[\s\S]*libraryStore\.findByRemote\(record\.server\.id, `\$\{series\.id\}`\)[\s\S]*addSelectedSeriesToLibrary\(\)[\s\S]*listVolumes\(series\.id\)[\s\S]*createSeriesComic\(record, series, chapters\)[\s\S]*upsertComicAndPersistLibraryStore[\s\S]*KomaActionButton\(\{[\s\S]*common_add_to_library/, 'Kavita browse page must let users add a whole series to the library without opening an individual chapter first')
