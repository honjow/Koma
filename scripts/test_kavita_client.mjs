...
assert.match(clientSource, /http\.request\(\{[\s\S]*method: 'GET'[\s\S]*url: this\.buildUrl\(path\)[\s\S]*headers: this\.buildHeaders\(headers\)/, 'Kavita client must delegate GET requests through the injected adapter')
