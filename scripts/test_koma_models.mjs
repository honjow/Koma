...
assert.match(
  indexSource,
  /private readerChapterIdForOpen\(comic: Comic, requestedChapterId\?: string\): string \| undefined \{[\s\S]*this\.readerSessionStore\.getProgress\(comic\.id\)[\s\S]*sortedReaderChapters\(comic\.chapters\)\[0\][\s\S]*private async ensureReaderSourceChapterPages\(comic: Comic, chapterId: string \| undefined\): Promise<boolean> \{[\s\S]*new SourceChapterPageHydrator\([\s\S]*this\.libraryStore,[\s\S]*this\.sourceRegistry,[\s\S]*this\.ensureLibraryPersistenceService\(\)[\s\S]*ensureChapterPages\(comic\.id, comic\.sourceRuntimeId, chapterId as string\)[\s\S]*return false[\s\S]*let sourcePagesReady = true[\s\S]*sourcePagesReady = await this\.ensureReaderSourceChapterPages\(comic, targetChapterId\)[\s\S]*let nextSessionConfig = comic === undefined \? MOCK_LIBRARY_READER_SESSION : this\.createReaderSessionConfigForOpen\(comic, targetChapterId\)[\s\S]*!sourcePagesReady && comic !== undefined && nextSessionConfig\.totalPages <= 0[\s\S]*createSourceUnavailableReaderSessionConfig\(comic, targetChapterId, nextSessionConfig\)[\s\S]*this\.readerSessionConfig = nextSessionConfig/,
  'Index openReader must hydrate source-backed target chapters, allow offline manifest fallback, and enter Reader with an honest source-unavailable placeholder when source pages cannot be hydrated',
)
assert.match(
  indexSource,
...
assert.match(
  chapterListSectionSource,
  /ChapterRow\(chapter: MangaChapterItem\)[\s\S]*Row\(\{ space: ThemeConstants\.SPACE_MD \}\) \{[\s\S]*Column\(\{ space: ThemeConstants\.SPACE_XS \}\)[\s\S]*Text\(this\.chapterTitle\(chapter\)\)[\s\S]*Text\(this\.chapterSubtitle\(chapter\)\)[\s\S]*\.layoutWeight\(1\)[\s\S]*\.onClick\(\(\) => \{[\s\S]*this\.onOpenChapter\(chapter\.id\)[\s\S]*KomaIconButton\(\{[\s\S]*arrow_down_to_line[\s\S]*this\.onDownloadChapter\(chapter\.id\)[\s\S]*Column\(\)[\s\S]*ForEach\(this\.visibleChapters\(\)[\s\S]*this\.ChapterRow\(chapter\)[\s\S]*Divider\(\)/,
