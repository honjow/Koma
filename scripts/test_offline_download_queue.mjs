...
assert.match(downloadsPageSource, /DownloadRow\(entry: OfflineDownloadQueueEntry\)[\s\S]*Row\(\{ space: ThemeConstants\.SPACE_MD \}\) \{[\s\S]*Text\(this\.formatEntryStatus\(entry\)\)[\s\S]*\.onClick\(\(\) => \{[\s\S]*this\.openDownloadEntry\(entry\)[\s\S]*Row\(\{ space: ThemeConstants\.SPACE_MD \}\) \{[\s\S]*this\.retryDownload\(entry\)[\s\S]*this\.confirmRemoveDownload\(entry\)/, 'DownloadsPage must keep row navigation on the queue item content while preserving retry/remove button actions')
