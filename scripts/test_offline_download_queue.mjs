...
const downloadNotificationRouteSmokePath = resolve(root, 'scripts/run_download_notification_route_smoke.sh')
...
const downloadNotificationRouteSmokeSource = readFileSync(downloadNotificationRouteSmokePath, 'utf8')
...
assert.match(queueStoreSource, /OFFLINE_DOWNLOAD_FAILURE_REASON_CODES: string\[\] = \[[\s\S]*'paused'[\s\S]*'wifi_only'[\s\S]*'download_interrupted'[\s\S]*'manifest_malformed'[\s\S]*'identity_mismatch'[\s\S]*'unsafe_page_path'[\s\S]*'outside_chapter_dir'[\s\S]*'page_index_invalid'[\s\S]*'page_size_mismatch'[\s\S]*'page_hash_mismatch'[\s\S]*'comic_missing'[\s\S]*'page_fetch_failed'[\s\S]*\]/, 'queue store must keep a bounded allowlist of persisted failure reason codes, including safe manifest validation reasons')
...
assert.match(queueStoreSource, /OFFLINE_DOWNLOAD_FAILURE_REASON_CODES: string\[\] = \[[\s\S]*'source_not_installed'[\s\S]*'source_lookup_failed'[\s\S]*'source_disabled'[\s\S]*'source_pages_failed'[\s\S]*\]/, 'queue store must preserve bounded source page hydration failure reason codes for blocked download rows')
...
assert.match(queueStoreSource, /validation\.reasonCode === 'status_not_reader_ready' \?[\s\S]*validation\.manifest\?\.failureReasonCode \?\? 'manifest_unavailable'/, 'queue reconcile must preserve manifest failure reason codes after a reconciled failed manifest becomes not reader-ready')
...
assert.match(queueStoreSource, /loadReconciledSnapshot\(\): OfflineDownloadQueueEntry\[\] \{[\s\S]*reconciledEntries\(this\.loadDocument\(\), false\)\.entries[\s\S]*private reconciledEntries\(document: OfflineDownloadQueueDocument, persistManifestState: boolean\)[\s\S]*persistManifestState \?[\s\S]*reconcileDownloadedChapter\(entry\.comicId, entry\.chapterId\)[\s\S]*validateDownloadedChapter\(entry\.comicId, entry\.chapterId\)/, 'queue store must expose a read-only reconciled snapshot for Library filters while keeping manifest writes inside explicit reconciliation')
assert.match(queueStoreSource, /reconcileWithManifests\(\): OfflineDownloadQueueEntry\[\] \{[\s\S]*reconcileDownloadedChapter\(entry\.comicId, entry\.chapterId\)[\s\S]*OfflineDownloadedChapterStatus\.DOWNLOADED[\s\S]*OfflineDownloadedChapterStatus\.PARTIAL[\s\S]*entry\.status === OfflineDownloadStatus\.DOWNLOADING[\s\S]*download_interrupted[\s\S]*OfflineDownloadStatus\.FAILED/, 'queue store must persist manifest reconciliation and downgrade corrupt, missing, or interrupted running manifests into failed queue rows')
...
assert.match(offlineDownloadStoreSource, /export interface OfflineDownloadedPage\s*{[\s\S]*size: number[\s\S]*contentHash\?: string[\s\S]*}/, 'offline downloaded pages must carry an optional content hash for newly written page files')
assert.match(offlineDownloadStoreSource, /export interface OfflineDownloadedPage\s*{[\s\S]*width\?: number[\s\S]*height\?: number[\s\S]*localPath: string/, 'offline downloaded pages must carry optional reader image dimensions')
assert.match(offlineDownloadStoreSource, /function manifestIntegrityPayload[\s\S]*page\.pageIndex[\s\S]*page\.pageId[\s\S]*page\.fileName[\s\S]*page\.size[\s\S]*page\.contentHash[\s\S]*createManifestIntegrityHash/, 'offline manifest integrity must be deterministic from bounded identity and page metadata')
...
assert.doesNotMatch(extractTopLevelFunction(offlineDownloadStoreSource, 'manifestIntegrityPayload'), /page\.width|page\.height/, 'offline manifest integrity must not depend on optional page dimensions so older downloaded manifests stay valid')
assert.match(offlineDownloadStoreSource, /function normalizeOptionalPageDimension\(value: number \| undefined\): number \| undefined \{[\s\S]*Number\.isFinite\(value\)[\s\S]*value <= 0[\s\S]*return undefined[\s\S]*saveManifest\(manifest: OfflineChapterDownloadManifest\)[\s\S]*width: normalizeOptionalPageDimension\(page\.width\)[\s\S]*height: normalizeOptionalPageDimension\(page\.height\)/, 'offline manifest saves must preserve valid optional page dimensions without making dimensions part of integrity')
assert.match(offlineDownloadStoreSource, /createOfflineDownloadFileContentHash\(page\.localPath\) !== page\.contentHash[\s\S]*reasonCode: 'page_hash_mismatch'/, 'offline manifest validation must reject same-size page file corruption when a content hash is present')
assert.match(offlineDownloadStoreSource, /validateDownloadedChapter\([\s\S]*comicId: ComicId,[\s\S]*chapterId: string,[\s\S]*options\?: OfflineDownloadValidationOptions[\s\S]*manifest_missing[\s\S]*manifest_malformed[\s\S]*integrity_mismatch[\s\S]*page_hash_mismatch[\s\S]*page_file_missing/, 'offline store must expose reader-usable manifest discovery with missing, corrupt, and partial reasons')
assert.match(offlineDownloadStoreSource, /options\?\.validateContentHash !== false[\s\S]*createOfflineDownloadFileContentHash\(page\.localPath\) !== page\.contentHash/, 'offline manifest validation must keep hash checking enabled by default while allowing explicit lightweight checks')
...
assert.match(offlineDownloadStoreSource, /reconcileDownloadedChapter\(comicId: ComicId, chapterId: string\): OfflineDownloadManifestValidation \{[\s\S]*validateDownloadedChapter\(comicId, chapterId\)[\s\S]*OfflineDownloadedChapterStatus\.CORRUPT[\s\S]*return validation[\s\S]*OfflineDownloadedChapterStatus\.PARTIAL[\s\S]*status: OfflineDownloadStatus\.PARTIAL[\s\S]*pages: manifest\.pages\.filter[\s\S]*failureReasonCode: validation\.reasonCode \?\? 'page_file_missing'[\s\S]*validation\.reasonCode === 'pages_missing'[\s\S]*OfflineDownloadStatus\.FAILED[\s\S]*download_interrupted/, 'offline store must persist partial and interrupted manifest reconciliation without masking corrupt manifests')
...
assert.match(offlineDownloadStoreSource, /resolveDownloadedPage\([\s\S]*comicId: ComicId,[\s\S]*chapterId: string,[\s\S]*pageId: string,[\s\S]*pageIndex: number,[\s\S]*options\?: OfflineDownloadValidationOptions[\s\S]*validateDownloadedChapter\(comicId, chapterId, options\)[\s\S]*matchesDownloadedPage\(item, pageId, pageIndex\)/, 'offline reader page resolution must pass validation options through to avoid accidental main-thread full-chapter hashing')
...
assert.match(readerPageSourceAdapterSource, /validateDownloadedChapter\(config\.comicId, config\.chapterId, \{[\s\S]*validateContentHash: false,[\s\S]*\}\)/, 'reader offline page resolution must not re-hash downloaded chapter files on each render')
assert.match(readerPageSourceAdapterSource, /resolveDownloadedPage\(config\.comicId, config\.chapterId, pageId, pageIndex, \{[\s\S]*validateContentHash: false,[\s\S]*\}\)/, 'reader offline page path lookup must keep the same lightweight validation mode as the initial offline manifest check')
assert.match(readerPageSourceAdapterSource, /localPath === undefined[\s\S]*offlineOnly && isOfflineManifestReaderOwned\(validation\)[\s\S]*createReaderOfflineUnavailableSource\(config, pageIndex\)[\s\S]*return undefined[\s\S]*createReaderSourceRuntimeRenderSource/, 'reader must stop at an honest offline placeholder for partial or corrupt downloaded chapters only when offline-only is active')
assert.match(readerPageSourceAdapterSource, /validation\.manifest === undefined && validation\.reasonCode === 'manifest_missing'[\s\S]*return undefined/, 'reader must allow normal online fallback when no offline manifest exists')
assert.match(readerPageSourceAdapterSource, /interface SourceRuntimeImageHeaderRule[\s\S]*sourceRuntimeId: string[\s\S]*headersRef: string[\s\S]*headers: ReaderRemoteImageHeaders/, 'reader source adapter must model host-owned source image header refs')
assert.match(readerPageSourceAdapterSource, /export function installSourceRuntimeImageHeaders\([\s\S]*sourceRuntimeId: string,[\s\S]*headersRef: string,[\s\S]*headers: ReaderRemoteImageHeaders[\s\S]*sourceRuntimeImageHeaderRules/, 'reader source adapter must expose a host-owned image header profile installer')
assert.match(readerPageSourceAdapterSource, /resolveSourceRuntimeImageHeaders\(sourceRuntimeId, headersRef\)[\s\S]*payload\.requiresAuth === true && \(headersRef === undefined \|\| hostHeaders === undefined\)[\s\S]*missing_headers_ref/, 'source image requests requiring auth must fail closed unless the host has installed the referenced header profile')
assert.match(readerPageSourceAdapterSource, /const resolvedHeaders = mergeReaderRemoteImageHeaders\(headers, hostHeaders\)[\s\S]*headers: resolvedHeaders/, 'source image request resolution must return merged source-safe headers plus host-owned image header refs')
assert.match(readerPageSourceAdapterSource, /fetchReaderRemoteSourceBytes\(source: ReaderPageRenderSource\)[\s\S]*resolveSourceRuntimeImageRequest\(source\.sourceRuntimeId, source\.pageId, source\.pageUri\)[\s\S]*fetchReaderRemoteImageBytes\(resolved\.url, resolved\.headers\)[\s\S]*fetchAndCacheReaderRemoteSource\(source: ReaderPageRenderSource\)[\s\S]*fetchAndCacheReaderRemoteImage\(resolved\.url, resolved\.headers, resolved\.cacheKeySeed\)/, 'reader and download remote fetches must use resolved source image request headers')
...
assert.match(offlineDownloadServiceSource, /const reusablePage = reusableDownloadedPage\(reusablePages, pageId, index\)[\s\S]*contentHash: reusablePage\.contentHash[\s\S]*continue/, 'partial download retry must preserve reused page content hashes so resumed manifests keep same-size corruption detection')
assert.match(offlineDownloadServiceSource, /createOfflineDownloadStableByteHash\(new Uint8Array\(bytes\)\)[\s\S]*contentHash: writtenPage\.contentHash/, 'offline download service must persist content hashes for newly written remote pages')
assert.match(offlineDownloadServiceSource, /let pageFetchFailed = false[\s\S]*try \{[\s\S]*writtenPage = await this\.writePage\(source, localPath\)[\s\S]*catch \(error\) \{[\s\S]*pageFetchFailed = true[\s\S]*step=download_page_failed[\s\S]*continue[\s\S]*manifest\.failureReasonCode = pageFetchFailed \? 'page_fetch_failed'/, 'offline download service must continue after single-page fetch failures and report partial chapters with page_fetch_failed')
assert.match(offlineDownloadServiceSource, /validateDownloadedChapter\(comic\.id, chapterId, \{ validateContentHash: false \}\)[\s\S]*finalValidation\.status !== OfflineDownloadedChapterStatus\.DOWNLOADED[\s\S]*finalValidation\.availablePageCount !== finalValidation\.pageCount/, 'download completion validation must avoid re-hashing freshly written page files on the UI thread')
...
assert.match(offlineDownloadServiceSource, /export function canRunOfflineDownloadQueueNow\(preferences: OfflineDownloadQueuePreferences\): boolean[\s\S]*!preferences\.isPaused[\s\S]*!preferences\.wifiOnly \|\| isDefaultNetworkWifi\(\)[\s\S]*canDownloadNow\(preferences: OfflineDownloadQueuePreferences\): boolean[\s\S]*canRunOfflineDownloadQueueNow\(preferences\)/, 'download service must expose and reuse a shared paused/Wi-Fi-only gate before fetching pages')
assert.match(offlineDownloadServiceSource, /export function downloadBlockedReason\(preferences: OfflineDownloadQueuePreferences\): string \{[\s\S]*preferences\.isPaused \? 'paused' : 'wifi_only'[\s\S]*failureReasonCode = blockedReason[\s\S]*failureReasonCode = preferences\.isPaused \? 'paused' : 'wifi_only'/, 'download service must record paused versus Wi-Fi-only queue wait reasons using the latest start guard state')
...
assert.doesNotMatch(settingsPageSource, /download-notification-test|sendDownloadNotificationTest|download_notification_test_comic|settings_download_notification_test_sent/, 'Settings must not expose a user-facing test notification action')
assert.match(downloadNotificationUiSmokeSource, /hvigorw[\s\S]*assembleHap[\s\S]*aa start[\s\S]*click_from_layout[\s\S]*Settings[\s\S]*click_after_scrolls[\s\S]*Downloads[\s\S]*下载[\s\S]*capture_layout "download-notification-settings"[\s\S]*test notification row is visible[\s\S]*notification permission row is missing/, 'download notification UI smoke must install Koma, drive Settings > Downloads, and assert the real notification row remains without the test action')
assert.doesNotMatch(downloadNotificationUiSmokeSource, /dev\.sh|settings_download_notification_test_sent|hilog -x -e "download_notification_test"/, 'macOS download notification smoke must not use dev.sh or the removed notification-test hilog path')
assert.ok(downloadNotificationUiSmokeSource.split('\n').every((line) => {
  return !/click_after_scrolls/.test(line) || (!/Send test notification/.test(line) && !/发送测试通知/.test(line))
}), 'macOS download notification smoke must not drive the removed test notification UI')
assert.match(downloadNotificationRouteSmokeSource, /aa start[\s\S]*--ps koma\.launchRoute downloads[\s\S]*Downloads\|下载管理[\s\S]*Rescan downloads\|重扫下载[\s\S]*No downloads yet\|暂无下载任务/, 'download notification route smoke must launch the same downloads route used by notification taps and verify the real Downloads page')
assert.match(downloadNotificationRouteSmokeSource, /capture_layout\(\)[\s\S]*dumpLayout[\s\S]*\|\| return 1[\s\S]*file recv[\s\S]*\|\| return 1[\s\S]*\[ -s "\$artifact_dir\/\$name-layout\.json" \][\s\S]*if capture_layout "\$name" && assert_layout_contains_any/, 'download notification route smoke must fail cleanly when HDC cannot capture layout evidence')
assert.doesNotMatch(downloadNotificationRouteSmokeSource, /dev\.sh/, 'macOS download notification route smoke must not use dev.sh')
...
assert.match(indexSource, /name === RouteName\.DOWNLOADS[\s\S]*DownloadsPage\(\{[\s\S]*onOpenReader:\s*\(comicId: ComicId, chapterId\?: string\) => \{[\s\S]*this\.openReader\(comicId, chapterId\)/, 'Index must wire DownloadsPage downloaded chapter reads into the shared reader opener')
...
assert.match(downloadsPageSource, /queueStateText\(\):\s*string[\s\S]*downloads_queue_state_paused[\s\S]*!canRunOfflineDownloadQueueNow\(this\.preferences\)[\s\S]*downloads_status_waiting_wifi[\s\S]*downloads_queue_state_running[\s\S]*downloads_queue_state_queued[\s\S]*failed[\s\S]*blocked[\s\S]*concurrency/, 'DownloadsPage must show paused, waiting-for-Wi-Fi, running, queued, failed, blocked, and concurrency state text')
...
assert.match(downloadsPageSource, /downloads_filter_chip[\s\S]*icon:\s*\$r\('sys\.symbol\.chevron_down'\)[\s\S]*this\.FilterMenu\(\)/, 'DownloadsPage filter control must use a shared symbol chevron instead of text glyphs')
assert.match(downloadsPageSource, /downloads_concurrency_chip[\s\S]*icon:\s*\$r\('sys\.symbol\.chevron_down'\)[\s\S]*this\.ConcurrencyMenu\(\)/, 'DownloadsPage concurrency control must use a shared symbol chevron instead of text glyphs')
assert.match(downloadsPageSource, /setWifiOnly\(wifiOnly: boolean\): void[\s\S]*this\.queueStore\(\)\.setWifiOnly\(wifiOnly\)[\s\S]*this\.loadQueue\(false\)/, 'DownloadsPage must persist Wi-Fi-only queue preference without forcing manifest reconciliation')
assert.match(downloadsPageSource, /WifiOnlyControl\(\)[\s\S]*settings_row_download_wifi_only_title[\s\S]*Toggle\(\{ type: ToggleType\.Switch, isOn: this\.preferences\.wifiOnly \}\)[\s\S]*this\.setWifiOnly\(isOn\)[\s\S]*QueueControls\(\)[\s\S]*this\.WifiOnlyControl\(\)/, 'DownloadsPage must expose Wi-Fi-only downloads as a switch in queue controls')
assert.match(downloadsPageSource, /import \{ canRunOfflineDownloadQueueNow, OfflineDownloadService \} from '\.\.\/model\/OfflineDownloadService'[\s\S]*canStartQueueWork\(\): boolean[\s\S]*downloads_toast_queue_paused[\s\S]*!canRunOfflineDownloadQueueNow\(this\.preferences\)[\s\S]*downloads_status_waiting_wifi/, 'DownloadsPage must reuse the download service network gate before starting foreground queue work')
assert.match(downloadsPageSource, /retryDownload\(entry: OfflineDownloadQueueEntry\)[\s\S]*!this\.canStartQueueWork\(\)[\s\S]*retryFailedDownloads\(\)[\s\S]*!this\.canStartQueueWork\(\)[\s\S]*startQueuedDownloads\(\)[\s\S]*!this\.canStartQueueWork\(\)[\s\S]*resumeQueuedDownloads\(\)[\s\S]*resumeAllQueuedWork\(\)[\s\S]*!this\.canStartQueueWork\(\)/, 'DownloadsPage single retry, batch retry, start, and resume must fail closed when paused or waiting for Wi-Fi')
...
assert.match(downloadsPageSource, /startQueuedDownloads\(\):\s*void[\s\S]*!this\.canStartQueueWork\(\)[\s\S]*const targets = this\.queuedEntries\(\)[\s\S]*this\.runStartBatch\(targets\)/, 'DownloadsPage must expose a foreground start action for already queued downloads')
...
assert.match(downloadsPageSource, /import \{ RouterHelper \} from '\.\.\/common\/RouterHelper'/, 'DownloadsPage must use the shared app router for queue row navigation')
assert.match(downloadsPageSource, /openDownloadEntry\(entry: OfflineDownloadQueueEntry\): void \{[\s\S]*libraryStore\?\.getComic\(entry\.comicId\)[\s\S]*this\.blockUnavailableEntry\(entry\)[\s\S]*downloads_toast_entry_unavailable[\s\S]*comic\.sourceRuntimeId\?\.trim\(\)[\s\S]*comic\.remoteResourceId\?\.trim\(\)[\s\S]*RouterHelper\.pushMangaDetail\(\{ mangaId: sourceMangaId, sourceId \}\)[\s\S]*RouterHelper\.pushMangaDetail\(\{ mangaId: entry\.comicId \}\)/, 'DownloadsPage queue rows must reopen source-backed manga detail by source id and block stale missing-comic rows')
assert.match(downloadsPageSource, /DownloadRow\(entry: OfflineDownloadQueueEntry\)[\s\S]*ConciseListRow\(\{[\s\S]*title: this\.rowTitle\(entry\)[\s\S]*subtitle: this\.rowDetailSubtitle\(entry\)[\s\S]*trailingText: this\.formatEntryStatus\(entry\)[\s\S]*action: \(\) => \{[\s\S]*this\.openDownloadEntry\(entry\)[\s\S]*Row\(\{ space: ThemeConstants\.SPACE_MD \}\) \{[\s\S]*this\.retryDownload\(entry\)[\s\S]*this\.confirmRemoveDownload\(entry\)/, 'DownloadsPage must keep HDS row navigation on the queue item content while preserving retry/remove button actions')
assert.match(downloadsPageSource, /onOpenReader:\s*\(comicId: string, chapterId\?: string\) => void/, 'DownloadsPage must accept a shared reader-open callback')
assert.match(downloadsPageSource, /canRead\(entry: OfflineDownloadQueueEntry\): boolean \{[\s\S]*entry\.status === OfflineDownloadStatus\.DOWNLOADED[\s\S]*OfflineDownloadStatus\.PARTIAL/, 'DownloadsPage must expose direct reading for downloaded and partially available offline rows')
assert.match(downloadsPageSource, /openDownloadedChapter\(entry: OfflineDownloadQueueEntry\): void \{[\s\S]*!this\.canRead\(entry\)[\s\S]*libraryStore\?\.getComic\(entry\.comicId\)[\s\S]*this\.blockUnavailableEntry\(entry\)[\s\S]*this\.onOpenReader\(entry\.comicId, entry\.chapterId\)/, 'DownloadsPage direct read must route downloaded chapters through the shared reader opener and block stale rows')
assert.match(downloadsPageSource, /if \(this\.canRead\(entry\)\) \{[\s\S]*KomaActionButton\(\{[\s\S]*label: s\('common_read'\)[\s\S]*this\.openDownloadedChapter\(entry\)/, 'DownloadsPage readable offline rows must expose a direct read action')
...
assert.match(downloadsPageSource, /retryDownload\(entry: OfflineDownloadQueueEntry\)[\s\S]*OfflineDownloadService[\s\S]*downloadChapter/, 'DownloadsPage must retry recoverable entries through OfflineDownloadService')
assert.match(downloadsPageSource, /canRetry\([\s\S]*OfflineDownloadStatus\.FAILED[\s\S]*OfflineDownloadStatus\.PARTIAL[\s\S]*this\.isRecoverableBlockedEntry\(entry\)/, 'DownloadsPage must expose retry for failed, partial, and recoverable blocked rows')
assert.match(downloadsPageSource, /isRecoverableBlockedEntry\(entry: OfflineDownloadQueueEntry\): boolean \{[\s\S]*OfflineDownloadStatus\.BLOCKED[\s\S]*failureReasonCode === 'pages_missing'/, 'DownloadsPage must limit blocked retry to rows that can recover after page hydration')
assert.match(downloadsPageSource, /isRecoverableBlockedEntry\(entry: OfflineDownloadQueueEntry\): boolean \{[\s\S]*failureReasonCode === 'source_pages_failed'[\s\S]*failureReasonCode === 'source_lookup_failed'[\s\S]*failureReasonCode === 'source_disabled'/, 'DownloadsPage must allow retry for recoverable source page hydration failures after a source/runtime issue is fixed')
assert.match(downloadsPageSource, /retryableEntries\(\):\s*OfflineDownloadQueueEntry\[\][\s\S]*this\.visibleEntries\(\)\.filter[\s\S]*this\.canRetry\(entry\)/, 'DownloadsPage batch retry must reuse canRetry so only recoverable blocked rows are included')
assert.match(downloadsPageSource, /retryFailedDownloads\(\)[\s\S]*const targets = this\.retryableEntries\(\)[\s\S]*this\.runRetryBatch\(targets\)/, 'DownloadsPage must expose batch retry for failed, partial, and recoverable blocked rows')
assert.match(downloadsPageSource, /retryFailedDownloads\(\)[\s\S]*!this\.canStartQueueWork\(\)/, 'DownloadsPage must not start retry work while paused or waiting for Wi-Fi')
...
assert.match(downloadsPageSource, /confirmRemoveCompletedFailedDownloads\(\): void[\s\S]*const targets = this\.removableSettledEntries\(\)[\s\S]*downloads_cleanup_title[\s\S]*downloads_cleanup_message[\s\S]*primaryButton:[\s\S]*common_clear[\s\S]*this\.removeCompletedFailedDownloads\(\)/, 'DownloadsPage batch cleanup must require confirmation before deleting local downloaded files')
...
assert.match(downloadsPageSource, /runForegroundDownloadBatch\(targets: OfflineDownloadQueueEntry\[\]\): Promise<number>[\s\S]*entry\.status === OfflineDownloadStatus\.BLOCKED && !this\.isRecoverableBlockedEntry\(entry\)[\s\S]*continue/, 'DownloadsPage batch worker must skip unrecoverable blocked rows but allow pages-missing rows to retry after hydration')
assert.match(downloadsPageSource, /blockedHelpText\(entry: OfflineDownloadQueueEntry\)[\s\S]*failureReasonCode === 'pages_missing'[\s\S]*downloads_blocked_pages_missing/, 'DownloadsPage must explain pages_missing blocked rows while still allowing retry after the detail page hydrates pages')
assert.match(downloadsPageSource, /blockedHelpText\(entry: OfflineDownloadQueueEntry\)[\s\S]*failureReasonCode === 'source_pages_failed'[\s\S]*downloads_blocked_source_pages_failed[\s\S]*failureReasonCode === 'source_lookup_failed'[\s\S]*downloads_blocked_source_lookup_failed[\s\S]*failureReasonCode === 'source_disabled'[\s\S]*downloads_blocked_source_disabled[\s\S]*failureReasonCode === 'source_not_installed'[\s\S]*downloads_blocked_source_not_installed/, 'DownloadsPage must explain source hydration blocked rows with source-specific recovery copy')
assert.match(offlineDownloadServiceSource, /blockChapterDownload\(comic: Comic, chapterId: string, reasonCode: string[\s\S]*OfflineDownloadStatus\.BLOCKED[\s\S]*manifest\.failureReasonCode = reasonCode[\s\S]*this\.store\.saveManifest[\s\S]*this\.queueStore\.upsert\(this\.queueStore\.fromManifest/, 'OfflineDownloadService must persist source hydration failures into manifest-backed blocked queue rows')
assert.match(downloadsPageSource, /blockSourcePagesUnavailable\([\s\S]*new OfflineDownloadService\([\s\S]*this\.queueStore\(\)[\s\S]*service\.blockChapterDownload\(comic, entry\.chapterId, reasonCode/, 'DownloadsPage must use the shared offline download service to persist source hydration blocked rows')
assert.match(downloadsPageSource, /runRetry\(comic: Comic, entry: OfflineDownloadQueueEntry\): Promise<void> \{[\s\S]*const hydration = await this\.ensureSourcePagesForDownload\(comic, entry\.chapterId\)[\s\S]*if \(!hydration\.hydrated\) \{[\s\S]*this\.blockSourcePagesUnavailable\(comic, entry, hydration\.reasonCode \?\? 'pages_missing'\)[\s\S]*return[\s\S]*service\.downloadChapter/, 'DownloadsPage single retry must not continue into OfflineDownloadService when source page hydration fails')
assert.match(downloadsPageSource, /runForegroundDownloadBatch\(targets: OfflineDownloadQueueEntry\[\]\): Promise<number>[\s\S]*const hydration = await this\.ensureSourcePagesForDownload\(comic, entry\.chapterId\)[\s\S]*if \(!hydration\.hydrated\) \{[\s\S]*this\.blockSourcePagesUnavailable\(comic, entry, hydration\.reasonCode \?\? 'pages_missing'\)[\s\S]*continue[\s\S]*service\.downloadChapter/, 'DownloadsPage batch workers must not continue into OfflineDownloadService when source page hydration fails')
...
assert.match(downloadsPageSource, /confirmRemoveDownload\(entry: OfflineDownloadQueueEntry\): void[\s\S]*downloads_remove_title[\s\S]*downloads_remove_message[\s\S]*primaryButton:[\s\S]*common_remove[\s\S]*this\.removeDownload\(entry\)/, 'DownloadsPage single download removal must require confirmation before deleting local downloaded files')
assert.match(downloadsPageSource, /downloads_action_cleanup_finished_failed[\s\S]*this\.confirmRemoveCompletedFailedDownloads\(\)[\s\S]*label: s\('common_remove'\)[\s\S]*this\.confirmRemoveDownload\(entry\)/, 'DownloadsPage destructive cleanup and remove buttons must route through confirmation handlers')
...
assert.match(chapterListSectionSource, /const INITIAL_VISIBLE_CHAPTER_LIMIT: number = 80[\s\S]*@Local private visibleChapterLimit: number = INITIAL_VISIBLE_CHAPTER_LIMIT/, 'ChapterListSection must cap initial chapter rendering for large real-source chapter lists')
assert.match(chapterListSectionSource, /visibleChapterIds\(\):\s*string\[\][\s\S]*this\.visibleChapters\(\)\.map/, 'ChapterListSection batch actions must operate on the currently rendered visible chapter list')
assert.match(chapterListSectionSource, /visibleChapters\(\): MangaChapterItem\[\][\s\S]*this\.sortedChapters\.slice\(0, this\.visibleChapterLimit\)[\s\S]*showMoreChapters\(\): void[\s\S]*Math\.min\(this\.sortedChapters\.length, this\.visibleChapterLimit \+ VISIBLE_CHAPTER_INCREMENT\)/, 'ChapterListSection must expose a bounded load-more path instead of rendering every source chapter at once')
assert.match(chapterListSectionSource, /chapterSubtitle\(chapter: MangaChapterItem\)[\s\S]*chapter\.pageCount !== undefined[\s\S]*chapter_list_page_count/, 'Chapter rows must surface source-provided page counts when available')
assert.match(chapterListSectionSource, /SectionActionsMenu\(\)[\s\S]*ChapterBatchDownloadMode\.ALL_VISIBLE[\s\S]*ChapterBatchDownloadMode\.FAILED_ONLY[\s\S]*ChapterBatchDownloadMode\.NOT_DOWNLOADED/, 'ChapterListSection must expose all visible/failed/not downloaded batch actions')
...
assert.match(mangaDetailModelsSource, /export interface MangaChapterItem \{[\s\S]*pageCount\?: number[\s\S]*parseMangaChapterFromSourceItem[\s\S]*item\['page_count'\][\s\S]*item\['pageCount'\][\s\S]*item\['pages'\]/, 'source chapter parser must preserve pageCount from real source responses')
assert.match(mangaDetailPageSource, /decorateChapterStates\(chapters: MangaChapterItem\[\]\)[\s\S]*pageCount: chapter\.pageCount[\s\S]*comicFromSourceManga\(manga: MangaDetail, chapters: MangaChapterItem\[\], comicId: string\)[\s\S]*pageCount: chapter\.pageCount \?\? 0[\s\S]*comicChapters\.reduce\(\(total: number, chapter: Chapter\): number => total \+ chapter\.pageCount, 0\)/, 'MangaDetail must carry source chapter pageCount into library chapters and comic totals before page hydration')
assert.match(mangaDetailPageSource, /handleDownloadChapter\(chapterId\?: string\)[\s\S]*const comic = this\.libraryStore\?\.getComic\(this\.currentComicId\(\)\)[\s\S]*const hydration = await this\.ensureSourceChapterPages\(resolvedChapterId\)[\s\S]*if \(!hydration\.hydrated\) \{[\s\S]*service\.blockChapterDownload\(comic, resolvedChapterId, hydration\.reasonCode \?\? 'pages_missing'[\s\S]*return[\s\S]*await installReaderRemoteHeadersForComic/, 'MangaDetailPage single chapter download must persist source hydration failures as blocked queue rows before OfflineDownloadService')
assert.match(mangaDetailPageSource, /batchDownloadChapterIds\(chapterIds: string\[\], mode: ChapterBatchDownloadMode\)[\s\S]*ChapterBatchDownloadMode\.FAILED_ONLY[\s\S]*OfflineDownloadStatus\.FAILED[\s\S]*OfflineDownloadStatus\.PARTIAL[\s\S]*this\.isRecoverableBlockedDownload\(summary\)[\s\S]*ChapterBatchDownloadMode\.NOT_DOWNLOADED[\s\S]*OfflineDownloadStatus\.DOWNLOADED[\s\S]*isRecoverableBlockedDownload\(summary: OfflineDownloadSummary \| undefined\): boolean \{[\s\S]*OfflineDownloadStatus\.BLOCKED[\s\S]*failureReasonCode === 'pages_missing'[\s\S]*failureReasonCode === 'source_pages_failed'[\s\S]*failureReasonCode === 'source_lookup_failed'[\s\S]*failureReasonCode === 'source_disabled'/, 'MangaDetailPage must filter visible chapter batch actions by existing durable summaries and retry recoverable source-page blocked rows after hydration')
assert.match(mangaDetailPageSource, /handleDownloadVisibleChapters\(chapterIds: string\[\], mode: ChapterBatchDownloadMode\)[\s\S]*const hydration = await this\.ensureSourceChapterPages\(chapterId\)[\s\S]*if \(!hydration\.hydrated\) \{[\s\S]*service\.blockChapterDownload\(comic, chapterId, hydration\.reasonCode \?\? 'pages_missing'[\s\S]*continue[\s\S]*service\.downloadChapter\(comic, chapterId/, 'MangaDetailPage batch download must block source hydration failures before OfflineDownloadService')
assert.match(mangaDetailPageSource, /ensureSourceChapterPages\(chapterId: string\): Promise<SourceChapterPageHydrationResult>[\s\S]*new SourceChapterPageHydrator\([\s\S]*ensureChapterPages\(this\.currentComicId\(\), this\.manga\.sourceId, chapterId\)[\s\S]*return result/, 'MangaDetailPage source page hydration must delegate to the shared hydrator and return the full success or reason state')
assert.match(sourceChapterPageHydratorSource, /ensureChapterPages\([\s\S]*chapter\.pages\.length > 0[\s\S]*hydrated: true[\s\S]*try \{[\s\S]*pages = await this\.fetchChapterPages[\s\S]*reasonCode: 'source_pages_failed'[\s\S]*pages\.length === 0[\s\S]*hydrated: false[\s\S]*reasonCode: 'pages_missing'/, 'SourceChapterPageHydrator must report success/failure without pretending empty pages or thrown get_pages calls are valid')
assert.match(mangaDetailPageSource, /step=batch_pages_unavailable failed=true reason=\$\{hydration\.reasonCode \?\? 'source_pages_missing'\}/, 'MangaDetailPage batch source hydration failures must log the bounded hydrator reason code')
...
function stableByteHash(bytes) {
  ...
  for (let index = 0; index < bytes.length; index += 1) {
    ...
    hashA = Math.imul((hashA ^ value) >>> 0, 16777619) >>> 0
    hashB = Math.imul((hashB ^ (value + index)) >>> 0, 16777619) >>> 0
  }
  return `${hashA.toString(16).padStart(8, '0')}${hashB.toString(16).padStart(8, '0')}`
}
...
function manifestIntegrityPayload(manifest) {
  const pages = manifest.pages.slice().sort((left, right) => left.pageIndex - right.pageIndex).map((page) => {
    return [page.pageIndex, page.pageId, page.fileName, Math.max(0, Math.floor(page.size)), page.contentHash ?? ''].join(':')
  })
  return [
    1,
    manifest.sourceKind ?? '',
...
function validateFixtureManifest(path) {
  ...
  for (const page of manifest.pages) {
    if (existsSync(page.localPath)) {
      ...
      if ((page.contentHash ?? '').trim().length > 0) {
        const bytes = readFileSync(page.localPath)
        if (stableByteHash(bytes) !== page.contentHash) {
          return { status: 'corrupt', reasonCode: 'page_hash_mismatch' }
        }
      }
...
try {
  ...
  const completeManifest = stampManifest({
    ...
    updatedAt: 1,
    pages: [
      { pageId: 'page-a', pageIndex: 0, fileName: '00001-a.jpg', localPath: firstPage, size: 6, contentHash: stableByteHash(readFileSync(firstPage)), updatedAt: 1 },
      { pageId: 'page-b', pageIndex: 1, fileName: '00002-b.jpg', localPath: secondPage, size: 7, contentHash: stableByteHash(readFileSync(secondPage)), updatedAt: 1 },
    ],
    integrityHash: '',
  })
  ...
  writeFileSync(firstPage, 'page-z')
  assert.deepEqual(validateFixtureManifest(manifestPath), {
    status: 'corrupt',
    reasonCode: 'page_hash_mismatch',
  }, 'same-size page file corruption must be rejected when contentHash is present')
  writeFileSync(firstPage, 'page-a')
