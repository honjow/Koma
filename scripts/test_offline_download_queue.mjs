...
assert.match(downloadsPageSource, /canRead\(entry: OfflineDownloadQueueEntry\): boolean \{[\s\S]*entry\.status === OfflineDownloadStatus\.DOWNLOADED/, 'DownloadsPage must expose direct reading only for fully downloaded rows')
...
assert.match(downloadsPageSource, /if \(this\.canRead\(entry\)\) \{[\s\S]*KomaActionButton\(\{[\s\S]*label: s\('common_read'\)[\s\S]*this\.openDownloadedChapter\(entry\)/, 'DownloadsPage downloaded rows must expose a direct read action')
