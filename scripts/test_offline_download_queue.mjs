...
assert.match(queueStoreSource, /OFFLINE_DOWNLOAD_FAILURE_REASON_CODES: string\[\] = \[[\s\S]*'paused'[\s\S]*'wifi_only'[\s\S]*'download_interrupted'[\s\S]*'page_fetch_failed'[\s\S]*\]/, 'queue store must keep a bounded allowlist of persisted failure reason codes')
assert.match(queueStoreSource, /function normalizeFailureReasonCode\(value: string \| undefined\): string \| undefined \{[\s\S]*OFFLINE_DOWNLOAD_FAILURE_REASON_CODES\.indexOf\(value\) >= 0 \? value : undefined[\s\S]*const failureReasonCode = normalizeFailureReasonCode\(entry\.failureReasonCode\)/, 'queue entry normalization must drop unknown failure reason text before it can reach UI or notifications')
