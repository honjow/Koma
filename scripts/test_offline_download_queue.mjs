...
assert.match(queueStoreSource, /OFFLINE_DOWNLOAD_FAILURE_REASON_CODES: string\[\] = \[[\s\S]*'source_not_installed'[\s\S]*'source_lookup_failed'[\s\S]*'source_pages_failed'[\s\S]*\]/, 'queue store must preserve bounded source page hydration failure reason codes for blocked download rows')
...
assert.match(downloadsPageSource, /isRecoverableBlockedEntry\(entry: OfflineDownloadQueueEntry\): boolean \{[\s\S]*failureReasonCode === 'source_pages_failed'[\s\S]*failureReasonCode === 'source_lookup_failed'/, 'DownloadsPage must allow retry for recoverable source page hydration failures after a source/runtime issue is fixed')
