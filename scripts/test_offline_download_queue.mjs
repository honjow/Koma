...
assert.match(queueStoreSource, /entryFromManifestValidation\(validation: OfflineDownloadManifestValidation\)[\s\S]*OfflineDownloadedChapterStatus\.DOWNLOADED[\s\S]*OfflineDownloadStatus\.DOWNLOADED[\s\S]*OfflineDownloadedChapterStatus\.PARTIAL[\s\S]*OfflineDownloadStatus\.PARTIAL[\s\S]*OfflineDownloadStatus\.FAILED/, 'queue recovery must map downloaded, partial, and corrupt manifest validation into honest queue statuses')
...
assert.match(offlineDownloadServiceSource, /const initialPreferences = this\.queueStore\.loadPreferences\(\)[\s\S]*this\.queueStore\.upsert\([\s\S]*if \(!this\.canDownloadNow\(initialPreferences\) \|\| !this\.canDownloadNow\(this\.queueStore\.loadPreferences\(\)\)\)/, 'download service start guard must not depend on preferences after the initial queue upsert only')
assert.match(offlineDownloadServiceSource, /canDownloadNow\(this\.queueStore\.loadPreferences\(\)\)[\s\S]*OfflineDownloadStatus\.QUEUED/, 'download service must honor paused or non-Wi-Fi foreground queue state before starting work')
...
assert.match(offlineDownloadServiceSource, /failureReasonCode = initialPreferences\.isPaused \? 'paused' : 'wifi_only'[\s\S]*failureReasonCode = preferences\.isPaused \? 'paused' : 'wifi_only'/, 'download service must record paused versus Wi-Fi-only queue wait reasons')
