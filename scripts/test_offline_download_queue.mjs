...
assert.match(queueStoreSource, /OFFLINE_DOWNLOAD_FAILURE_REASON_CODES: string\[\] = \[[\s\S]*'paused'[\s\S]*'wifi_only'[\s\S]*'download_interrupted'[\s\S]*'manifest_malformed'[\s\S]*'identity_mismatch'[\s\S]*'unsafe_page_path'[\s\S]*'outside_chapter_dir'[\s\S]*'page_index_invalid'[\s\S]*'page_size_mismatch'[\s\S]*'page_fetch_failed'[\s\S]*\]/, 'queue store must keep a bounded allowlist of persisted failure reason codes, including safe manifest validation reasons')
