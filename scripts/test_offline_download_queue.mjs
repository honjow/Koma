...
assert.match(mangaDetailPageSource, /ensureSourceChapterPages\(chapterId: string\): Promise<boolean>[\s\S]*new SourceChapterPageHydrator\([\s\S]*ensureChapterPages\(this\.currentComicId\(\), this\.manga\.sourceId, chapterId\)[\s\S]*return result\.hydrated/, 'MangaDetailPage source page hydration must delegate to the shared hydrator and return its success state')
assert.match(sourceChapterPageHydratorSource, /ensureChapterPages\([\s\S]*chapter\.pages\.length > 0[\s\S]*hydrated: true[\s\S]*const pages = await this\.fetchChapterPages[\s\S]*pages\.length === 0[\s\S]*hydrated: false[\s\S]*reasonCode: 'pages_missing'/, 'SourceChapterPageHydrator must report success/failure without pretending empty pages are valid')
