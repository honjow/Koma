...
assert.match(chapterListSectionSource, /Row\(\{ space: ThemeConstants\.SPACE_MD \}\) \{[\s\S]*Column\(\{ space: ThemeConstants\.SPACE_XS \}\)[\s\S]*\.layoutWeight\(1\)[\s\S]*\.onClick\(\(\) => \{[\s\S]*this\.onOpenChapter\(chapter\.id\)[\s\S]*Button\('↓'\)[\s\S]*this\.onDownloadChapter\(chapter\.id\)/, 'Chapter open handler must be isolated to the left content area before the download button')
