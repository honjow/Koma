...
KS_FILE     = Path(os.environ.get("KOMA_KS_FILE", SIGNING_MATERIALS_DIR / "xiaobai.p12"))
KS_ALIAS    = "xiaobai"
KS_PWD      = "xiaobai123"
CSR         = Path(os.environ.get("KOMA_CSR_FILE", SIGNING_MATERIALS_DIR / "xiaobai.csr"))

CERT_FILE    = Path(os.environ.get("KOMA_CERT_FILE", SIGNING_MATERIALS_DIR / f"{CERT_NAME}.cer"))
PROFILE_FILE = Path(os.environ.get("KOMA_PROFILE_FILE", SIGNING_MATERIALS_DIR / f"{CERT_NAME}.p7b"))
AUTH_FILE    = Path.home() / "Documents/hap_installer/userInfo.json"

# 设备选择缓存文件及有效期（7 天）
DEVICE_CACHE_FILE = Path.home() / ".cache" / "koma_device.json"
...
def ensure_device(auth, udid):
    ...
    if not any(d.get("udid") == udid for d in device_list):
        ...
        api("https://connect-api.cloud.huawei.com/api/cps/device-manage/v1/device/add",
            data={"deviceName": f"koma-dev-{udid[:10]}", "udid": udid, "deviceType": 4},
            auth=auth)
        device_list = api("https://connect-api.cloud.huawei.com/api/cps/device-manage/v1/device/list?start=1&pageSize=100&encodeFlag=0",
                          method="GET", auth=auth).get("list", [])
...
def ensure_profile(auth, cert_id, device_ids):
    profile_name = f"koma-debug_{BUNDLE_NAME.replace('.', '_')}"
...
def main():
    ...
    non_interactive = "--non-interactive" in args or os.environ.get("KOMA_SIGN_NONINTERACTIVE") == "1"
